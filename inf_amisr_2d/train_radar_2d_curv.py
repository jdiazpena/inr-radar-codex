...
Loss:
    total_loss = data_loss + lambda_curv_eff * curv_loss
...
No gradient loss in this version.
No temporal loss.
No uncertainty weighting yet.
"""
...
def make_collocation_pool(
    ...
    """
    Create normalized collocation points for curvature loss.

    These points have no measured Ne value.
    They are only used for curvature regularization.
    """
...
def spatial_curvature_loss(
    ...
    The model output is normalized log10(Ne).
    The coordinates are normalized x/y in [-1, 1].

    This computes:

        mean( fxx^2 + 2 fxy^2 + fyy^2 )

    where derivatives are with respect to normalized coordinates.

    This penalizes rapid changes in slope.
    It allows broad linear trends more easily than a gradient penalty.
    """

    coords_col = coords_col.detach().clone().requires_grad_(True)

    pred = model(coords_col)  # [N, 1]
...
def plot_grid_prediction(
    ...
    ax.set_xlabel("x east [km]")
    ax.set_ylabel("y north [km]")
    ax.set_title("SIREN reconstruction with curvature loss")
    ax.set_aspect("equal", adjustable="box")
    ax.grid(True, alpha=0.3)
...
def train(args: argparse.Namespace) -> None:
    ...
    # ------------------------------------------------------------
    # 2. Collocation pool for curvature loss
    # ------------------------------------------------------------
    ...
    print()
    print("Collocation points for curvature loss:")
    ...
    for step in pbar:
        ...
        total_loss = data_loss + curv_loss_weighted
        ...
        if step == 1 or step % args.log_every == 0 or step == args.num_steps:
            pbar.set_postfix_str(
                ...
                f"curv={curv_loss_raw.item():.2e} "
                f"lc={lambda_curv_eff:.1e} "
    ...
    # ------------------------------------------------------------
    # 6. Dense query grid for visualization only
    # ------------------------------------------------------------
    measured_xy = df[["x_km", "y_km"]].to_numpy(dtype=float)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="outputs/radar_2d_curvature",
    )
