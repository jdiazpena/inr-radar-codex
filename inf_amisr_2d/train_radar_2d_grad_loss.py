...
def plot_history(
    ...
    ax.plot(hist["step"], hist["grad_loss"], marker="o", markersize=3, label="grad")
...
def train(args: argparse.Namespace) -> None:
    ...
    print()
    print("Collocation points for gradient loss:")
    ...
    for step in pbar:
        ...
        if args.lambda_grad > 0.0:
            ...
            grad_loss = spatial_gradient_loss(
                model=model,
                coords_col=coords_col,
            )

            lambda_grad_eff = ramp_weight(
                target_weight=args.lambda_grad,
                step=step,
                num_steps=args.num_steps,
                ramp_frac=args.reg_ramp_frac,
            )
        else:
            grad_loss = data_loss.new_tensor(0.0)
            lambda_grad_eff = 0.0

        loss = data_loss + lambda_grad_eff * grad_loss
        ...
        if step == 1 or step % args.summary_every == 0 or step == args.num_steps:
            ...
            row = {
                ...
                "lambda_grad_eff": float(lambda_grad_eff),
                "rmse_log10": metrics["rmse"],
                "mae_log10": metrics["mae"],
                "bias_log10": metrics["bias"],
                "max_abs_log10": metrics["max_abs"],
        ...
        if step == 1 or step % args.log_every == 0 or step == args.num_steps:
            ...
            pbar.set_postfix_str(
                ...
                f"grad={grad_loss.item():.2e} "
                f"lg={lambda_grad_eff:.1e} "
                f"rmse={latest_metrics['rmse']:.2e}"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--num_collocation", type=int, default=512)
    parser.add_argument("--collocation_grid_nx", type=int, default=120)
    parser.add_argument("--collocation_grid_ny", type=int, default=120)
...
def make_collocation_pool(
    ...
    """
    Create a pool of normalized collocation points for smoothness loss.

    These points have no data target.
    They are only used for the gradient regularization term.

    Returns:
        collocation_coords: torch.Tensor [Ncol, 2], normalized coordinates
        nearest_radius_km: radius used for the support mask
        valid_fraction: fraction of grid points kept
    """
...
def sample_collocation_points(
    ...
    """
    Randomly sample collocation points from the precomputed pool.
    """
...
def spatial_gradient_loss(
    ...
    coords_col:
        normalized [x, y] collocation points, shape [N, 2]

    This computes:
        mean( df/dx_norm ^ 2 + df/dy_norm ^ 2 )

    The derivative is with respect to normalized coordinates.
    """

    coords_col = coords_col.detach().clone().requires_grad_(True)

    pred_col = model(coords_col)

    grads = torch.autograd.grad(
        outputs=pred_col,
        inputs=coords_col,
        grad_outputs=torch.ones_like(pred_col),
        create_graph=True,
        retain_graph=True,
        only_inputs=True,
    )[0]

    grad_loss = torch.mean(torch.sum(grads ** 2, dim=1))
...
def ramp_weight(
    ...
    """
    Gradually turn on a regularization weight.

    If ramp_frac = 0.2, the weight ramps from 0 to target_weight
    over the first 20 percent of training.
    """
