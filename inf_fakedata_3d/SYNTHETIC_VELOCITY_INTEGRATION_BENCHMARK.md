...
SCOPE=pilot STAGE=generate ./run_velocity_integration_benchmark.sh
SCOPE=pilot STAGE=train ./run_velocity_integration_benchmark.sh
SCOPE=pilot STAGE=analyze ./run_velocity_integration_benchmark.sh
SCOPE=pilot STAGE=check ./run_velocity_integration_benchmark.sh
```

Use `STAGE=all` only when a complete scope should run sequentially. Existing
complete outputs are skipped. Incomplete output directories stop the run rather
than being silently overwritten.

For a quick pipeline test:

```bash
SCOPE=smoke STAGE=all NUM_STEPS=20 ./run_velocity_integration_benchmark.sh --cpu
```
