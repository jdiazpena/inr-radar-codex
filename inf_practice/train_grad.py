...
def compute_target_gradients(
    ...
    Coordinates live in [-1, 1], so the spacing is:
...
def predict_gradients_in_chunks(
    ...
    """
    Evaluate the gradient of the model output with respect to input coords.
...
def train(args: argparse.Namespace) -> None:
    ...
    # ------------------------------------------------------------------
    # 2) Load dataset and precompute gradient targets
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 4) Choose one anchor pixel to fix the additive-constant ambiguity
    # ------------------------------------------------------------------
    # Use the center pixel
    center_row = args.sidelength // 2
    center_col = args.sidelength // 2
    anchor_idx = center_row * args.sidelength + center_col

    anchor_coords = full_coords[anchor_idx:anchor_idx + 1]
    anchor_value = full_values[anchor_idx:anchor_idx + 1]
    ...
    for step in range(1, args.num_steps + 1):
        ...
        # Anchor value loss: fixes the constant-offset ambiguity
        pred_anchor = model(anchor_coords)
        anchor_loss = F.mse_loss(pred_anchor, anchor_value)

        total_loss = grad_loss + args.anchor_weight * anchor_loss
        ...
        if step % args.log_every == 0 or step == 1:
            pct = 100.0 * step / args.num_steps
            msg = (
                f"step {step:6d}/{args.num_steps:6d} "
                f"({pct:6.2f}%) | "
                f"grad_mse {grad_loss.item():.8f} | "
                f"anchor_mse {anchor_loss.item():.8f} | "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--batch_size", type=int, default=4096,
                        help="Gradient supervision is heavier than value-only training.")
    parser.add_argument("--num_steps", type=int, default=10000)
    parser.add_argument("--anchor_weight", type=float, default=1.0,
                        help="Weight for the single-pixel anchor loss.")
    ...
    parser.add_argument("--output_dir", type=str, default="outputs_grad")
